...
%l=ones(256); %start from a lattice of +1
...
lattice = IsingUpdate(lattice,betaJ,h,1);%equilibrate the model, might need to adjust '100'
disp("Now writing to gif")
for n = 1:frames
    % Draw plot
    lattice = IsingUpdate(lattice,betaJ,h,sweepsPerPrint);
    %imshow((l+1)/2);%Uncomment this to view movie live
